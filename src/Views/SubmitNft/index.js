...
export default function SubmitNFT() {
  ...
  const register = async () => {
    ...
    if (files.instance.size > NFT_AVATAR_MAX_FILE_SIZE)
      return message.warn('You can only upload up to 4MB');
  ...
  return (
    <>
      <LoadingModal title={content} open={visible} />
                      ...
                      renderItem={(item) => (
                        <List.Item
                          className={classNames({
                            selected: item.addressToken === acceptContractAddress,
                          })}
            ...
            {!activeTabIsApprover && (
              <>
                <button
                  ...
                  onClick={register}
                  disabled={!web3.utils.isAddress(contractAddress)}
                >
                  Submit
                </button>
              </>
            ...
            {activeTabIsApprover && (
              <>
                <button
                  className='submit-button'
                  onClick={accept}
                  disabled={!web3.utils.isAddress(acceptContractAddress)}
                >
                  Approve
                </button>
              </>
