...
export default function Profile() {
  ...
  const { erc721Tokens, isLoadingErc721, listErc721Address } = useSelector((state) => state);
  const { address } = useParams();

  const [isCopied, setIsCopied] = useState(false);

  useEffect(() => {
    if (!!listErc721Address && !!address) {
      dispatch(getNFTsOfOwner(listErc721Address, address));
    }
  }, [listErc721Address, address, dispatch]);
