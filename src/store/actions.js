import { parseBalance, listTokensERC721OfOwner, listTokensERC115OfOwner } from 'utils/helper';
...
export const SET_LIST_ERC_721_ADDRESS = 'SET_LIST_ERC_721_ADDRESS';
export const INIT_ERC1155 = 'INIT_ERC1155';
export const SET_LIST_ERC_1155_ADDRESS = 'SET_LIST_ERC_1155_ADDRESS';
export const initERC721 = (acceptedNftsAddress) => async (dispatch, getState) => {
  const { web3, nftList, walletAddress } = getState();
  let erc721Instances = [];
  let listErc721Address = [];
  let erc1155Instances = [];
  let listErc1155Address = [];
  if (!!acceptedNftsAddress) {
    for (let i = 0; i < acceptedNftsAddress.length; i++) {
      ...
      if (is1155) {
        ...
        listErc1155Address.push(acceptedNftsAddress[i]);
      } else {
        erc721Instances.push(new web3.eth.Contract(ERC721.abi, acceptedNftsAddress[i]));
        listErc721Address.push(acceptedNftsAddress[i]);
      }
    }

    dispatch({ type: INIT_ERC721, erc721Instances });
    dispatch({ type: SET_LIST_ERC_721_ADDRESS, listErc721Address });
    dispatch({ type: INIT_ERC1155, erc1155Instances });
    dispatch({ type: SET_LIST_ERC_1155_ADDRESS, listErc1155Address });
    dispatch(getNFTsOfOwner(listErc721Address, walletAddress));
  }
};

export const GET_OWNED_ERC721 = 'GET_OWNED_ERC721';
export const GET_OWNED_ERC1155 = 'GET_OWNED_ERC1155';
export const getNFTsOfOwner = (acceptedNftsAddress, walletAddress) => async (
  dispatch,
  getState
) => {
  ...
  //     let ERC721token = {};
  //     const listIdToken = await listTokensERC721OfOwner, listTokensERC115OfOwner(
  //       instance._address,
  //       walletAddress,
  //       contractAddress.Market
  //     );
  //     let balanceOfOwner = listIdToken.length;
  //     if (balanceOfOwner > 0) {
  //       ERC721token.tokenIdOwner = listIdToken;
  ...
  //         let token = listIdToken[i];
  //         token.index = listIdToken[i].tokenID;
  ...
  const erc721Tokens = await listTokensERC721OfOwner(acceptedNftsAddress, walletAddress);

  const erc1155Tokens = await listTokensERC115OfOwner(acceptedNftsAddress, walletAddress);

  dispatch({ type: GET_OWNED_ERC721, erc721Tokens });
  dispatch({ type: GET_OWNED_ERC1155, erc1155Tokens });
...
export const createSellOrder = (nftAddress, tokenId, price, token) => async (
  ...
  const { market, walletAddress, web3, listErc721Address } = getState();
  try {
...
export const buyNft = (orderDetail) => async (dispatch, getState) => {
  const { market, walletAddress, listErc721Address, chainId, web3 } = getState();
  let link = null;
  let value = 0;
...
export const cancelSellOrder = (orderDetail) => async (dispatch, getState) => {
  const { market, walletAddress, listErc721Address } = getState();
  try {
    await dispatch(setLoadingTx(true));
    await market.methods
...
export const generateERC721NFT = (collectionId, tokenUri) => async (dispatch, getState) => {
  const { web3, chainId, walletAddress, listErc721Address, userCollections } = getState();
  contractAddress = getContractAddress(chainId);
  let erc721Instance;
  if (collectionId !== -1) {
      ...
      var getInfoCampaign = (instance) => {
        return new Promise(async (resolve) => {
          ...
          if (!!walletAddress) {
            ...
            listTokensERC115OfOwner(instanceNFT, walletAddress, contractAddress.Market)).owned;
